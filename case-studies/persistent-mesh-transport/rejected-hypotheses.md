...
A first read of `CRT-001` suggests every node ships the cert/SNI tell, which
...
radius is a node *class*, not the fleet — so `CRT-001` is held at **S1**
...
untyped-shell ownership is recorded as `CRT-003` (S3); it is context for the
